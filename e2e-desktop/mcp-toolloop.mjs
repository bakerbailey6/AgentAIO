...
const MCP_CMD='node /home/user/AgentAIO/node_modules/@modelcontextprotocol/server-everything/dist/index.js stdio'
...
try{
  ...
  await nameInput.setValue('McpBot')
  const sel=await browser.$('select'); await sel.selectByVisibleText('llama3.2:3b')
  await click({tag:'button',text:'Create'}); await waitText('McpBot',12000)
  // add the MCP server via the Store footer
  await clickUntil({sel:'aside button[aria-label="Store"]'},'MCP Servers')
  const fi=await browser.$('input[placeholder^="npx"]'); await fi.waitForExist({timeout:8000}); await fi.setValue(MCP_CMD)
  ...
  await kill()
  await browser.execute(()=>{const el=[...document.querySelectorAll('*')].find(e=>e.textContent.trim()==='McpBot'&&e.children.length===0);let n=el;for(let i=0;i<6&&n;i++){n.dispatchEvent(new MouseEvent('dblclick',{bubbles:true}));n=n.parentElement}})
  ...
  fs.writeFileSync('/tmp/shot-mcp.png', await browser.takeScreenshot(),'base64')
