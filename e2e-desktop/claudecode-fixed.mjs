...
try{
  ...
  // Project directory field should now appear for coding agents
  const pd=await browser.$('input[placeholder="/path/to/project"]')
  if(!(await pd.isExisting())){console.log('FAIL: project-directory field missing for coding agent');process.exit(1)}
  await pd.setValue('/tmp/coderdir')
  console.log('PASS: project-directory-field-present')
  await click({tag:'button',text:'Create'}); await waitText('CoderBot',15000)
  await kill()
  ...
  await ta.setValue('Reply with exactly: HELLO'); await browser.keys('Enter')
  const t0=Date.now();let got=''
  while(Date.now()-t0<70000){got=await body();if(/project directory/i.test(got))break;if((got.match(/HELLO/g)||[]).length>=2)break;await sleep(1500)}
  fs.writeFileSync('/tmp/shot-claudecode-fixed.png', await browser.takeScreenshot(),'base64')
  if(/project directory/i.test(got)) console.log('FAIL: still errors -> requires a project directory')
  else if((got.match(/HELLO/g)||[]).length>=2) console.log('PASS: claude-code-agent REAL response (HELLO) — no project-dir error')
  else console.log('PARTIAL: no project-dir error but no HELLO; tail='+got.slice(-160))
